...
var ZeroFraction = NewFraction(Zero, nil)
...
func (f *Fraction) ToSignificant(significantDigits uint, opt ...Option) string {
	f.opts = NewNumberOption(WithGroupSeparator('\xA0'), WithRoundingMode(RoundHalfUp))
	f.opts.Apply(opt...)
	...
	f.opts.Apply(WithRoundingPrecision(int(significantDigits)))
	if v, err := DecimalRound(d, f.opts); err == nil {
		d = v
	}
	return DecimalFormat(d, f.opts)
}
...
func (f *Fraction) ToFixed(decimalPlaces uint, opt ...Option) string {
	f.opts = NewNumberOption(WithGroupSeparator('\xA0'), WithRoundingMode(RoundHalfUp))
	f.opts.Apply(opt...)
	f.opts.Apply(WithDecimalPlaces(decimalPlaces))
