...
func NewPrice(baseCurrency, quoteCurrency *Currency, denominator, numerator *big.Int) *Price {
	return &Price{
		...
		Scalar: NewFraction(math.BigPow(Ten.Int64(), int64(baseCurrency.Decimals)),
			math.BigPow(Ten.Int64(), int64(quoteCurrency.Decimals))),
	}
}
...
func (p *Price) ToSignificant(significantDigits uint, opt ...Option) string {
	return p.Adjusted().ToSignificant(significantDigits, opt...)
}

func (p *Price) ToFixed(decimalPlaces uint, opt ...Option) string {
